...
// VAT defaulting
//
// The AI never sets vatEnabled/vatRate (not in its tool schema) — VAT is an
// explicit opt-in the user controls. This sets the initial state on newly
// generated proposals: on by default only if the workspace already has a
// VAT number saved in Branding, otherwise off. Applied once, after content
// is finalised (AI success or fallback), right before the proposal is
// inserted — never re-applied on later edits, so it won't stomp a user's
// manual toggle in the editor.
// ---------------------------------------------------------------------------

export function applyVatDefault(
  content: ProposalContent,
  brand: BrandSettings | null
): ProposalContent {
  const hasVatNumber = Boolean(brand?.vat_number?.trim());
  return {
    ...content,
