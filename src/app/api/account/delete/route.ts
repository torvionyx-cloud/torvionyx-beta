 ...
 *
 * Account deletion isn't built yet — this stub returns 501 so the "Delete
 * account" flow on the settings page can show a clear "not available yet"
...
export async function POST() {
  try {
    ...
    return NextResponse.json(
      { error: "Account deletion isn't available yet." },
      { status: 501 }
    );
  } catch (error) {
